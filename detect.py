"""
Recipe for training a word detection system on the dataset.
...
from ogi_prepare import prepare_aligned_ogi
...
class DetectBrain(sb.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        encoded_feats = self.modules.encoder(feats)
    ...
    def on_stage_start(self, stage, epoch):
        "Gets called when a stage (either training, validation) starts."
        if stage != sb.Stage.TRAIN:
    ...
    def freeze_modules(self):
        for key in self.hparams.frozen_module_keys:
            for p in self.modules[key].parameters():
...
def segmenting_collate_fn(examples):
    ...
    for example in examples:
        ...
        for seg_index in range(max_index):
            seg = compute_segment(example, seg_index, length)
            if seg is not None:
                segments.append(seg)
...
def compute_segment(example, seg_index, length):
    ...
    word_inputs = []
    post_count = 0
    for i, (start, end) in enumerate(example["splits"]):
...
def dataio_prep(hparams):
    ...
    @sb.utils.data_pipeline.takes("starts", "stops")
    @sb.utils.data_pipeline.provides("splits")
    def split_pipeline(starts, stops):
        starts = [float(f) for f in starts.strip().split()]
        stops = [float(f) for f in stops.strip().split()]
    ...
    for dataset in ["train", "valid"]: 
        datasets[dataset] = sb.dataio.dataset.DynamicItemDataset.from_json(
            ...
            dynamic_items=[audio_pipeline, word_pipeline, split_pipeline],
            output_keys=["id", "sig", "phonemes_encoded", "splits"],
        )
    ...
    return datasets, phoneme_encoder
...
if __name__ == "__main__":
    ...
    prepare_aligned_ogi(
        ...
        hparams["train_alignments"],
        hparams["valid_alignments"],
    )
    datasets, phoneme_encoder = dataio_prep(hparams)
    ...
    detect_brain.hparams.pretrained.collect_files(".")
    detect_brain.hparams.pretrained.load_collected(detect_brain.device)
    detect_brain.freeze_modules()
